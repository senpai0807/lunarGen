  ...
  const answers = await inquirer.prompt([
    ...
    const promises = nikeAccountsData.map(async (line, index) => {
        ...
        try {
            ...
            await cursor.click('button[class="nds-btn delete-account-button css-cqi5lw ex41m6f0 btn-secondary-dark "]');
            await page.waitForSelector('input[id="hasConfirmedDelete"]');
            await page.check('input[id="hasConfirmedDelete"]');
            await cursor.click('button[class="nds-btn confirm-delete-account css-1d2mqbf ex41m6f0 btn-secondary-dark  btn-responsive"]');
            await page.waitForSelector('h1[aria-label="Enter your email to join us or sign in."]');
            logger.info(`Task ${taskid}: Account Successfully Deleted...`);
